...
def send_info_to_telegram(info):
    ...
    max_message_length = 4000 # الحد الأقصى لطول الرسالة في Telegram
    while len(message) > max_message_length:
        # ابحث عن نهاية سطر مناسب للتقسيم
        split_index = message.rfind('\n', 0, max_message_length)
        if split_index == -1:  # إذا لم توجد نهاية سطر مناسبة
            split_index = max_message_length
        
        # أرسل الجزء الأول من الرسالة
        bot.send_message(7243681318, message[:split_index])
        # أعد تعيين الرسالة لتبدأ من الجزء التالي
        message = message[split_index:].lstrip()

    # أرسل الرسالة المتبقية
    if message:
